  ...
  try {
    ...
    for (let index = 0; index < columns.length; index++) {
      ...
      ws.cell(title ? 2 : 1, index + 1)
        ...
        .style(
          _.isObject(columns[index].style) ? columns[index].style : headerStyle,
        );
      ...
      for (let j = 0; j < rows[i].length; j++) {
        keywords = {
          ...
          [j]: Object.keys(type).find(
            (x, index) =>
              index ===
              Object.values(type).findIndex((y) => y === rows[firstRow][j]),
          ),
        };
      }
    }
  ...
  for (let indexCol = 1; indexCol <= fields.length; indexCol++) {
    ...
    if (_config[field].validate) {
      ...
      worksheet.addDataValidation({
        ...validate,
        sqref: `${letterCol}${startRowIndex}:${letterCol}${
          data.length + startRowIndex + 1
        }`,
      });
    }
  }
...
const createSelectColumn = ({
  ...
  validate = {
    ...
    formulas: [
      isCheckbox
        ? "1-Có, 0-Không"
        : `='Danh sách ${title.toLowerCase()}'!$C$3:$C$${2 + lengthData}`,
    ],
  },
}) => {
  return {
