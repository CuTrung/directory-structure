...
const templateHelper = require("../utils/template.util");
const BULLMQ = require("../../bullmq");

const sendToInside = async function (
  mail = {},
  TYPE = "OFFWORK",
  subitle = null,
) {
  ...
  try {
    ...
    if (users && users.length) {
      for (let i = 0; i < users.length; i++) {
        ...
        const resultMailUser = await requestMailUser
          ...
          .input(
            "USERNAME",
            apiHelper.getValueFromObject(users[i], "user_name"),
          )
    ...
    if (departments && departments.length) {
      for (let i = 0; i < departments.length; i++) {
        ...
        const resultMailUser = await requestMailUser
          ...
          .input(
            "DEPARTMENTID",
            apiHelper.getValueFromObject(departments[i], "user_name"),
          )
          .execute("SYS_MAILBOX_CreateMailUser_WebAdmin");
        const mailUserId = resultMailUser.recordset[0].RESULT;
        if (!mailUserId) {
    ...
    if (mailId) {
      BULLMQ.push({
        ...
        payload: {
          mail_id: mailId,
          user_send: apiHelper.getValueFromObject(mail, "user_name"),
        },
      });
    }
