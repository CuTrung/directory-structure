...
const apiHelper = require("../utils/api.util");
// check connect sms
const checkConnection = async () => {
  try {
    const signmh = CryptoJS.MD5(config.sendSms.pass).toString();
    const sign = CryptoJS.MD5(
      config.sendSms.loginName + "-" + signmh,
    ).toString();
    const requestArgs = {
    ...
    const resultService = await callSoap(
      url,
      wsdlOptions,
      requestArgs,
      "CheckConnection",
  ...
  try {
    ...
    if (checkConnectioncall.getData().data.Code !== "106") {
      return new ServiceResponse(
        false,
        RESPONSE_MSG.SENDSMS.SENDSMS_CHECKCONNECTION_FAILED,
      );
    }
    ...
    if (brandNameCall.getData().data.Code !== "106") {
      return new ServiceResponse(
        false,
        RESPONSE_MSG.SENDSMS.SENDSMS_GETBRANDNAME_FAILED,
      );
    }
    ...
    if (serviceTypeCall.getData().data.Code !== "106") {
      return new ServiceResponse(
        false,
        RESPONSE_MSG.SENDSMS.SENDSMS_GETSERVICETYPELIST_FAILED,
      );
    }
    const serviceType =
      serviceTypeCall.getData().data.ServiceTypeList.SubServiceType[1]
        .ServiceTypeId;
    const signmh =
      config.sendSms.loginName +
      "-" +
      pass +
      "-" +
      brandName +
