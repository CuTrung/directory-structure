...
const { convertEnvToArray } = require('../app/common/utils/array.util');
...
const addRules = () => {
    ...
    for (const envVar of envsRule) {
        ...
        result[envVar] = Joi.string().valid(valueValid).default(valueValid[defaultValueIndex]);
    }
    return result;
};
...
const initValidate = () => {
    ...
    const { error, value } = Joi.validate(listEnv, Joi.object(addRules()).unknown().required());
    return { error, value };
};
