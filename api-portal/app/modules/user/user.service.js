...
const getListUser = async (body = {}) => {
    try {
        const dataRecord = await execProcedure('test', { my_name: getValue(body, 'my_name', 2) });
        return new ServiceResponse(true, 'Lấy danh sách thành công', dataRecord.recordset);
    } catch (error) {
        return new ServiceResponse(false, error.message ?? 'Lấy danh sách thất bại', error);
    }
};
