...
const mergeObjectSameKey = (
  ...
  for (let item of list) {
    item = {
      ...item,
      ...keysWantToMerge.reduce(
        (a, v) => ({ ...a, [v]: Array.isArray(item[v]) ? item[v] : [item[v]] }),
        {},
      ),
    };
...
const convertObjectToArray = (obj = {}) =>
  isObject(obj) ? Object.entries(obj) : null;

const pickObject = (object, pick = []) =>
  isObject(object) ? _.pick(object, pick) : null;

const omitObject = (object, omit = []) =>
  isObject(object) ? _.omit(object, omit) : null;

const getValue = (object, key, defaultValue) =>
  pickObject(object, [key]) ?? defaultValue;
